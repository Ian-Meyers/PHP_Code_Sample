    ...
    public function updateUsersContactInfo($organizationId, $contactInfoListDTO)
    {
        ...
        foreach ($contactInfoList as $contactInfoDTO) {
            try {
                ...
                // This is to match the database layout for these objects
                foreach ($personContacts as $key => $contactInfoObject) {

                    try {
                        $dataProcessingExceptionHandlerLoop = new DataProcessingExceptionHandler();
                        $optionalError = null;
                        ...
                        $optionalError = $this->entityValidationService->validateDoctrineEntity($contactInfoObject, $dataProcessingExceptionHandlerLoop, null, false);

                        // Restore the optional parameter
                        $contactInfoObject = $this->entityValidationService->restoreErroredProperties($contactInfoObject, $cloneContactInfoObject, $dataProcessingExceptionHandlerLoop);
                        ...
                        // Throw error
                        if ($optionalError) {
                            $dataProcessingExceptionHandlerLoop = $optionalError;
                            if (count($dataProcessingExceptionHandlerLoop->getAllErrors()) > 0) {
                                throw $dataProcessingExceptionHandlerLoop;
                            }
                        }

                    } catch (DataProcessingExceptionHandler $dpeh) {
                        $dataProcessingExceptionHandler->copyErrorsIntoDataProcessingExceptionHandler($dpeh);
                    }
                }
                ...
                $errorDisplayArray= $dpeh->getPlainErrors();
                ...
                if (count($errorDisplayArray) > 0) {
                    $error[] = $this->dataProcessingUtilityService->buildErrorResponseArray($contactInfoArray, $errorDisplayArray, false, $this->jsonEntityFieldMap);
                }
            }
        }
        return [
